...
class Modelo_Holerite():
        ...
        def gerar_modelo(self):
                ...
                dados_funcionario = self.cadastro.Consulta(matricula)
                ...
                consultar_salario = (f"""
                                        SELECT descricao, taxa_comissao FROM cargos WHERE codigo_cargo = {cargo};
                        """)

                cursor.execute(consultar_salario)

                funcao = cursor.fetchall()[0][0]
                taxa_comissao = cursor.fetchall()[0][1]

                cnx.close()

                salario_base = self.holerite.salario_base
                base_de_calculo = self.holerite.base_de_calculo
                comissao = self.holerite.comissao
                ...
                salario_liquido = self.holerite.liquido_funcionario
